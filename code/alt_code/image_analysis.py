'''
    Adds each sub_image into split_images folders under each frame
'''
import numpy as np
import pandas as pd
    ...
    for d in scanner_dir:
	    ...
	    print "Splitting frame {}".format(frame)
            ...
            if frame_name in all_frames:
                ...
                split_path = join(dir_path, "split_images")
                if not exists(split_path):
                        makedirs(split_path)
                segment_images(frame_name, frame_path, split_path, xstep, ystep, win_size, df_mitosis, df_non_mitosis, cell_distance)
...
def segment_images(frame_name, frame_path, split_path, xstep, ystep, win_size, df_mitosis, df_non_mitosis, cell_distance):
    ...
    for x_cord in range(0, img_x, xstep):
        for y_cord in range(0, img_y, ystep):
            ...
            save_path = join(split_path, frame)
...
if __name__ == '__main__':
    ...
    path = '/home/ubuntu/capstone/ScannerA_tmp'
    create_imageset(path, 50, 50, 100, 30)
